...
function printStyles({ println }: { println: (str: string) => void }) {
  return () => {
    println(`<style>
    .statechart {
...
export function buildSvg({
  ...
  println(
    `<svg width="${width}" height="${height}" viewbox="0 0 ${width} ${height}" class="statechart"  xmlns="http://www.w3.org/2000/svg">`
  );
  printStyles({ println })();
